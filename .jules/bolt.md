...
## 2024-06-25 - Avoid flat_map.include?
**Learning:** Using `flat_map(&:property).include?(value)` evaluates the entire collection and allocates intermediate arrays, increasing garbage collection pressure.
**Action:** Use `.any? { |item| item.property.include?(value) }` instead to short-circuit and avoid allocating intermediate arrays.
