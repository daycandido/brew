...
**Learning:** Using `.map(&:property).any?` evaluates the entire collection and allocates an intermediate boolean array, increasing garbage collection pressure and preventing short-circuiting.
**Action:** Use `.any?(&:property)` or `.any? { |item| condition }` instead, which short-circuits upon finding a match and avoids intermediate allocations.
