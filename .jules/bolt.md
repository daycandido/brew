...
## 2026-09-25 - Avoid map.any?
**Learning:** Using `.map(&:property).any?` evaluates the entire collection and allocates an intermediate boolean array, increasing garbage collection pressure.
**Action:** Use `.any?(&:property)` instead, which short-circuits upon finding a match and avoids intermediate allocations.
