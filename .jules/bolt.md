...
**Learning:** Using `.select(&:property).map(&:value).include?(target)` evaluates the entire collection and allocates two intermediate arrays, increasing garbage collection pressure.
**Action:** Use `.any? { |item| item.property? && item.value == target }` instead, which short-circuits upon finding a match and avoids intermediate allocations.
