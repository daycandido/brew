...
RSpec.describe Tty do
  describe "::strip_ansi" do
    ...
    it "removes OSC 8 escape codes from a string" do
      expect(described_class.strip_ansi("\033]8;;http://example.com\033\\link\033]8;;\033\\")).to eq("link")
      expect(described_class.strip_ansi("\033]8;;http://example.com\alink\033]8;;\a")).to eq("link")
    end
  end

  describe "::hyperlink" do
    before do
      allow(Homebrew::EnvConfig).to receive_messages(no_color?: false, color?: false)
    end
    ...
    context "when $stdout is a TTY" do
      ...
      it "returns an OSC 8 hyperlink" do
        expect(described_class.hyperlink("link", "http://example.com"))
          .to eq("\033]8;;http://example.com\033\\link\033]8;;\033\\")
      end

      it "returns the text when HOMEBREW_NO_HYPERLINKS is set" do
        ENV["HOMEBREW_NO_HYPERLINKS"] = "1"
        expect(described_class.hyperlink("link", "http://example.com")).to eq("link")
