  ...
  describe "dumping" do
    ...
    context "when go is installed" do
      ...
      it "returns package list" do
        allow(Utils).to receive(:popen_read).with("go", "env", "GOBIN").and_return("")
        allow(Utils).to receive(:popen_read).with("go", "env", "GOPATH").and_return("/Users/test/go")
        ...
        allow(Utils).to receive(:popen_read).with("go", "version", "-m", "/Users/test/go/bin/crush", err: "/dev/null")
