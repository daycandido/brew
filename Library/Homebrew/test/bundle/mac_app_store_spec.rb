  ...
  describe "dumping" do
    ...
    context "when there is no apps" do
      before do
        ...
        allow(Utils).to receive(:popen_read).with(Pathname.new("mas"), "list", err: :close).and_return("")
      end
    ...
    context "with invalid app details" do
      ...
      before do
        described_class.reset!
        allow(described_class).to receive(:package_manager_executable).and_return(Pathname.new("mas"))
        allow(Utils).to receive(:popen_read).with(Pathname.new("mas"), "list",
                                                  err: :close).and_return(invalid_mas_output)
      end
    ...
    context "with the new format after mas-cli/mas#339" do
      ...
      before do
        described_class.reset!
        allow(described_class).to receive(:package_manager_executable).and_return(Pathname.new("mas"))
        allow(Utils).to receive(:popen_read).with(Pathname.new("mas"), "list", err: :close).and_return(new_mas_output)
      end
    ...
    context "when mas is installed" do
      ...
      describe ".outdated_app_ids" do
        it "returns app ids" do
          expect(Utils).to receive(:popen_read).with(Pathname.new("mas"), "outdated",
                                                     err: :close).and_return("foo 123")
          described_class.reset!
          described_class.outdated_app_ids
        end
      end
