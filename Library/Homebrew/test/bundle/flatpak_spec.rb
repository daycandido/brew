  ...
  describe "dumping" do
    ...
    context "when flatpak is installed", :needs_linux do
      ...
      it "returns remote URLs" do
        allow(Utils).to receive(:popen_read).with(anything, "remote-list", "--system", "--columns=name,url", err: "/dev/null")
      ...
      it "returns package list with remotes and URLs" do
        allow(Utils).to receive(:popen_read)
          .with(anything, "list", "--app", "--columns=application,origin", err: "/dev/null")
          .and_return("org.gnome.Calculator\tflathub\ncom.spotify.Client\tflathub\n")
        allow(Utils).to receive(:popen_read)
          .with(anything, "remote-list", "--system", "--columns=name,url", err: "/dev/null")
          .and_return("flathub\thttps://dl.flathub.org/repo/\n")
      ...
      it "handles packages without origin" do
        allow(Utils).to receive(:popen_read).with(anything, "list", "--app", "--columns=application,origin", err: "/dev/null")
