  ...
  class << self
    ...
    def clang_version
      @clang_version ||= T.let(
        if (path = locate("clang")) &&
           (build_version = Utils.safe_popen_read(path, "--version")[/(?:clang|LLVM) version (\d+\.\d(?:\.\d)?)/, 1])
    ...
    def clang_build_version
      @clang_build_version ||= T.let(
        if (path = locate("clang")) &&
          (build_version = Utils.safe_popen_read(path, "--version")[%r{clang(-| version [^ ]+ \(tags/RELEASE_)(\d{2,})}, 2])
          Version.new(build_version)
        else
          Version::NULL
        end, T.nilable(Version)
    ...
    def llvm_clang_build_version
      @llvm_clang_build_version ||= T.let(begin
        ...
        if path.executable? && (build_version = Utils.safe_popen_read(path, "--version")[/clang version (\d+\.\d\.\d)/, 1])
          Version.new(build_version)
        else
          Version::NULL
        end
    ...
    def gcc_version(cc = host_gcc_path.to_s)
      (@gcc_version ||= T.let({}, T.nilable(T::Hash[String, Version]))).fetch(cc) do
        ...
        version = if path &&
                     (build_version = Utils.safe_popen_read(path, "--version")[/gcc(?:(?:-\d+(?:\.\d)?)? \(.+\))? (\d+\.\d\.\d)/, 1])
          Version.new(build_version)
        else
          Version::NULL
        end
