          ...
          %w[lxml pynacl pyyaml].each do |resource_name|
            ...
            uses_from_macos_or_depends_on = (uses_from_macos_nodes + depends_on_nodes).filter_map do |node|
              if (dep = node.arguments.first).hash_type?
                ...
                # ⚡ Bolt: Optimized by replacing select.map.include? with any?
                # to avoid intermediate array allocations and short-circuit evaluation
                dep.keys.first.str_content if dep_types.any? do |dep_type|
                  dep_type.sym_type? && dep_type.value == :build
                end
              else
                dep.str_content
              end
