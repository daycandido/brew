      ...
      class << self
        ...
        def remote_urls
          ...
          @remote_urls = if (flatpak = package_manager_executable)
            output = Utils.popen_read(flatpak, "remote-list", "--system", "--columns=name,url", err: "/dev/null").chomp
            urls = {}
            output.split("\n").each do |line|
        ...
        def packages_with_remotes
          ...
          @packages_with_remotes = if (flatpak = package_manager_executable)
            ...
            output = Utils.popen_read(flatpak, "list", "--app", "--columns=application,origin", err: "/dev/null").chomp
